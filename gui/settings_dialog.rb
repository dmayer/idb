class SettingsDialog < Qt::Dialog
  ...
  def setup_port_forward
    ...
    $settings['remote_forwards'].each {|x|
      add_local_forward_to_list x['remote_port'], x['local_port'], x['local_host']
    }
  ...
  def setup_device_config
    ...
    @device_config_layout.addWidget @label_ssh_username, 0, 0
    @device_config_layout.addWidget @ssh_username, 0, 1

    # ssh password
    @label_ssh_password = Qt::Label.new  "SSH Password:", self, 0
    @ssh_password = Qt::LineEdit.new  $settings.ssh_password
    @device_config_layout.addWidget @label_ssh_password, 1, 0
    @device_config_layout.addWidget @ssh_password, 1, 1

    # ssh host
    @label_ssh_host = Qt::Label.new  "SSH Host:", self, 0
    @ssh_host = Qt::LineEdit.new  $settings.ssh_host
    @device_config_layout.addWidget @label_ssh_host, 2, 0
    @device_config_layout.addWidget @ssh_host, 2, 1

    # ssh port
    @label_ssh_port = Qt::Label.new  "SSH Port:", self, 0
    @ssh_port = Qt::LineEdit.new  $settings.ssh_port.to_s
    @device_config_layout.addWidget @label_ssh_port, 3, 0
    @device_config_layout.addWidget @ssh_port, 3, 1
